...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res.status(200)
    .json(new ApiResponse(200, user, " coverfImage updated successfully"))
})
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await db.users.aggregate([
    {
      $match: {
        // username = username?.toLowerCase()
        username: username
    ...
    {
      //total number of subscrubed channel and no of subscirber 
      $addFields: {
        SubscribersCount: {
          $size: "$subscribers"
        },
...
const getWatchHistory = asyncHandler(async (req, res) => {
  ...
  return res.staus(200).json(new ApiResponse(200,user[0].watchHistory,"watch history fetched successfully"))
})
